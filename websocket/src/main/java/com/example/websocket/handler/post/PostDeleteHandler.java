...
public class PostDeleteHandler {
    ...
    public void handleDeletePost(WebSocketSession session, JsonNode payload) {
        if (payload.hasNonNull(POST_ID) && payload.hasNonNull(USER_ID)) {
            ...
            }, error -> WebSocketErrorHandler.sendErrorMessage(session, "Error deleting post",
                    error));
        } else {
            WebSocketErrorHandler.sendErrorMessage(session, "Missing fields in post.delete payload",
                    null);
        }
    }
}
