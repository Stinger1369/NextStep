...
public class FollowHandler {
    ...
    public void handleFollowUser(WebSocketSession session, JsonNode payload) {
        ...
        if (payload.hasNonNull(USER_ID) && payload.hasNonNull(FOLLOW_ID)
            ...
            logger.info("User with userId: {} is following user with followId: {}", userId,
                    followId);
            userService.followUser(userId, followId, firstName, lastName).subscribe(unused -> {
                WebSocketErrorHandler.sendMessage(session, "user.follow.success", null);
                logger.info("User with userId: {} followed user with followId: {}", userId,
                        followId);

                // Create a notification
                String message = String.format("User %s started following you.", firstName);
                Notification notification =
                        new Notification(userId, firstName, lastName, message, content, "follow");

                notificationService.createNotification(notification).subscribe();
            }, error -> {
                logger.error("Error following user with userId: {} and followId: {}", userId,
                        followId, error);
                WebSocketErrorHandler.sendErrorMessage(session, "Error following user", error);
            });
        } else {
    ...
    public void handleUnfollowUser(WebSocketSession session, JsonNode payload) {
        ...
        if (payload.hasNonNull(USER_ID) && payload.hasNonNull(FOLLOW_ID)) {
            String userId = payload.get(USER_ID).asText();
            String followId = payload.get(FOLLOW_ID).asText();
            logger.info("User with userId: {} is unfollowing user with followId: {}", userId,
                    followId);
            userService.unfollowUser(userId, followId).subscribe(unused -> {
                WebSocketErrorHandler.sendMessage(session, "user.unfollow.success", null);
                logger.info("User with userId: {} unfollowed user with followId: {}", userId,
                        followId);

                // Create a notification
                String message = String.format("User %s stopped following you.", userId);
                Notification notification = new Notification(followId, "System", "Notification",
                        message, userId, "unfollow");
                notificationService.createNotification(notification).subscribe();
            }, error -> {
                logger.error("Error unfollowing user with userId: {} and followId: {}", userId,
                        followId, error);
                WebSocketErrorHandler.sendErrorMessage(session, "Error unfollowing user", error);
            });
        } else {
