...
public class NettyConfiguration {
        ...
        public RouterFunction<ServerResponse> route(ConversationController conversationController,
                ...
                return RouterFunctions.route()
                                ...
                                .GET("/users/{id}",
                                                request -> userController.getUserById(request.pathVariable("id"))
                                                                .flatMap(user -> ServerResponse.ok().bodyValue(user))
                                                                .switchIfEmpty(ServerResponse.notFound().build()))
                                .GET("/users",
                                                request -> ServerResponse.ok().body(userController.getAllUsers(),
                                                                User.class))
                                .PUT("/users/{id}",
                                                request -> request.bodyToMono(User.class)
                                                                .flatMap(user -> userController.updateUser(
                                                                                request.pathVariable("id"), user))
                                                                .flatMap(user -> ServerResponse.ok().bodyValue(user))
                                                                .switchIfEmpty(ServerResponse.notFound().build()))
                                .DELETE("/users/{id}",
                                                request -> userController.deleteUser(request.pathVariable("id"))
                                                                .then(ServerResponse.noContent().build()))
